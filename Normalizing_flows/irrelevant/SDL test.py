...
X = np.concatenate((X_train_z, X_valid_z, X_test_z)).T

scaledX = Scale(X)

sd, m, delta = SparseDictionary(X, scaledX, kernel=quadratic_kernel, tolerance=tol)

plt.plot(m)
plt.show()

plt.plot(delta)
plt.axhline(y=tol, linestyle='--', label='threshold')
plt.legend()
plt.show()

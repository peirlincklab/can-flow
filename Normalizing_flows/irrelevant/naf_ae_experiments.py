...
male_indices = np.where(x_confounders[:, 0].flatten() == 0)[0]
female_indices = np.where(x_confounders[:, 0].flatten() == 1)[0]
...
norm_flow = torch.load("flow_model_naf.pth")
...
synthetic_z = norm_flow.sample(x_conf=torch.tensor(x_confounders, dtype=torch.float32).to(device))
synthetic_z = synthetic_z[:, :50]
...
synthetic_z_train = norm_flow.sample(X_train_conf)
synthetic_z_train = synthetic_z_train[:, :50]
...
for i in range(NumValidSamples+NumTestSamples):
    for j in range(NumGen):
        ...
        synthetic_z_exp2 = norm_flow.sample(X_test_exp_conf[i].reshape(-1, 1).T)
        synthetic_z_exp2 = synthetic_z_exp2[:, :50]
...
z_synth_female = norm_flow.sample(generated_conf_samples)
z_synth_female = z_synth_female[:, :50]
...
z_synthetic_male_exp3 = norm_flow.sample(torch.tensor(generated_conf_samples_male, dtype=torch.float32).to(device))
z_synthetic_male_exp3 = z_synthetic_male_exp3[:, :50]
...
for conf_island in confs_isl_to_generate:
    ...
    z_synthetic_island = norm_flow.sample(conf_island)
    z_synthetic_island = z_synthetic_island[:, :50]

...
def loss_function(beta_value, dense_prediction, x_pointcloud, logvar, mean, epoch):
    ...
    if epoch < 470:
        ...
        loss_recon_ = chamfer_distance_test(dense_prediction, x_pointcloud)
...
def visualization(num_neighbors, x_pred, x_ref, title_pred, title_ref, num_patient, type_batch):
    ...
    plt.savefig(
        f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/Reconstruction/plots/{type_batch}/recon_{num_patient}.png")
    plt.close()
...
def chamfer_distance_test(x, y):
    """
    Compute Chamfer Distance between two point clouds.

    Args:
        x: (batch_size, n_points, 3) tensor representing the first point cloud
        y: (batch_size, m_points, 3) tensor representing the second point cloud

    Returns:
        Chamfer distance value for the batch.
    """
    x = x.unsqueeze(2)  # (batch_size, n_points, 1, 3)
    y = y.unsqueeze(1)  # (batch_size, 1, m_points, 3)

    dist_matrix = torch.norm(x - y, p=2, dim=3)  # (batch_size, n_points, m_points)

    # Compute minimum distances
    min_dist_x_to_y, _ = dist_matrix.min(dim=2)  # (batch_size, n_points)
    min_dist_y_to_x, _ = dist_matrix.min(dim=1)  # (batch_size, m_points)

    # Chamfer distance is the sum of the minimum distances from each point in both sets
    chamfer_loss = min_dist_x_to_y.mean() + min_dist_y_to_x.mean()
...
def results_errors(X_pred, X_ref, type_batch="train"):
    ...
    for i in range(X_pred.shape[0]):
        ...
        np.savetxt(f'/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/Reconstruction/'
...
def synthetic_generation(metadata_vector, num_samples, trained_vae):
    ...
    for i in range(generated_shapes.shape[0]):
        ...
        plt.savefig(
            f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/Generation/plots/generated_sample_{i}.png")
        plt.close()

        np.savetxt(f'/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/Generation/rawdata/generated_sample_{i}.txt',
...
fc_decoder_configs = [1024, 1024, 2704, False]
...
beta_vae_vals = np.linspace(1, 5, 5)

encoder = PCN_Encoder(latent_dim_output=encoder_output_dimension, conv_configs=conv, mlp_configs=mlp_to_encoder_output)
...
X_train_momenta = deformed[:TrainSamples, :, :num_dense].transpose((0, 2, 1))
X_valid_momenta = deformed[TrainSamples:(ValidSamples + TrainSamples), :, :num_dense].transpose((0, 2, 1))
X_test_momenta = deformed[(ValidSamples + TrainSamples):, :, :num_dense].transpose((0, 2, 1))
...
for epoch in range(epochs):
    ...
    for x_train, y_metadata in train_loader:
        ...
        loss_train, loss_recon = loss_function(beta_value=beta, dense_prediction=dense_pred,
                                               logvar=log_var, mean=mu, x_pointcloud=x_train, epoch=epoch)
    ...
    with torch.no_grad():
        for x_val, y_val in valid_loader:
            ...
            loss_valid, loss_recon_valid = loss_function(beta_value=beta, dense_prediction=dense_pred_val,
                                                         logvar=log_var_val, mean=mu_val, x_pointcloud=x_val, epoch=epoch)
...
if verbose:
    ...
    plt.axvline(x=230, linestyle='--', color='black')
    plt.axvline(x=380, linestyle='--', color='black')
    plt.axvline(x=530, linestyle='--', color='black')
    plt.axvline(x=680, linestyle='--', color='black')
    plt.xlabel("# Epochs")
    plt.ylabel("Loss function")
    plt.legend()
    plt.savefig(f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/training_error.png")
    plt.close()
    ...
    plt.ylabel("Loss function")
    plt.legend()
    plt.savefig(f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+PCN/validation_error.png")
    plt.close()
...
results_errors(X_pred=reconstruction_train, X_ref=X_train_momenta, type_batch="train")
results_errors(X_pred=reconstruction_test, X_ref=X_test_momenta, type_batch="test")

...
from Decoder_FullyConnected import *
from VAE import *
...
def loss_function(beta_value, dense_prediction, x_pointcloud, logvar, mean):
    ...
    loss_recon = loss_reconstruct(dense_prediction, x_pointcloud)
...
def visualization(num_neighbors, x_pred, x_ref, title_pred, title_ref, num_patient, type_batch):
    ...
    plt.savefig(
        f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/Reconstruction/plots/{type_batch}/recon_{num_patient}.png")
    plt.close()
...
def Chamfer_distance_batch(X_pred, X_ref):
    ...
    for i in range(X_pred.shape[0]):
        cd = chamfer_distance(X_pred[i], torch.tensor(X_ref[i], dtype=torch.float32))
        c_dists.append(cd.detach().numpy())
        pbar_cd.update()
    pbar_cd.close()
...
def results_errors(X_pred, X_ref, type_batch="train"):
    ...
    for i in range(X_pred.shape[0]):
        ...
        np.savetxt(f'/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/Reconstruction/'
...
def synthetic_generation(metadata_vector, num_samples, trained_vae):
    ...
    for i in range(generated_shapes.shape[0]):
        ...
        plt.savefig(
            f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/Generation/plots/generated_sample_{i}.png")
        plt.close()

        np.savetxt(
            f'/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/Generation/rawdata/generated_sample_{i}.txt',
...
fc_decoder_configs = [1024, 2048, 2730, True]
...
train_loader = DataLoader(dataset=dataset_train,
                          batch_size=int(batch_size_frac * TrainSamples), shuffle=True)
valid_loader = DataLoader(dataset=dataset_valid,
                          batch_size=int(batch_size_frac * ValidSamples), shuffle=True)

optimizer = torch.optim.Adam(vae.parameters(), lr=lrate)

### TODO: Check if I need to step the scheduler (or if I need the scheduler at all)
r_schedule = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.7)
...
for epoch in range(epochs):
    ...
    for x_train, y_metadata in train_loader:
        ...
        coarse_pred, dense_pred, mu, log_var = vae(x_train, y_metadata)
    ...
    with torch.no_grad():
        for x_val, y_val in valid_loader:
            coarse_pred_val, dense_pred_val, mu_val, log_var_val = vae(x_val, y_val)
...
if verbose:
    ...
    plt.legend()
    plt.savefig(f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/training_error.png")
    plt.close()
    ...
    plt.ylabel("Loss function")
    plt.legend()
    plt.savefig(f"/Users/konstantinoskevopoulos/Desktop/Generated_shapes/PCN+FNN/validation_error.png")
    plt.close()

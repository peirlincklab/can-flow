### This file performs the PCA experiment, to compare the performance of CAN-FLOW and cVAEs in the space of momenta
### We perform PCA on the space of momenta --> not the space scalar clinical phenotypes
...
### This file produces the results illustrated in Figure 9 and Figure B.13 of the manuscript
...
def visualize_1d_pca(real_rb, cnf_rb, cvae_rb, num_toplot):
    """
    Visualize one-dimensional marginal distributions of PCA coefficients for real and
    synthetic data.

    For each PCA dimension, the function estimates the probability
    density of the corresponding coefficient using Gaussian kernel density
    estimation. It then plots the KDE curves for the real data, CAN-FLOW, and the cVAE synthetic data

    Parameters
    ----------
    real_rb : np.ndarray
        PCA coefficients of the real data.
    cnf_rb : np.ndarray
        PCA coefficients of the synthetic data generated by CAN-FLOW.
    cvae_rb : np.ndarray
        PCA coefficients of the synthetic data generated by the cVAE.
    num_toplot : int
        Number of PCA modes to visualize. The function plots the first
        `num_toplot` modes.
    """
    ...
    for i in range(num_toplot):
        ...
        plt.figure()
        plt.plot(x_vae, d_vae, color='#0072B2')
        plt.fill_between(x_vae, d_vae, color='#0072B2', alpha=0.15)
        plt.plot(x_ref, d_ref, color='black')
        plt.fill_between(x_ref, d_ref, color='black', alpha=0.15)
        plt.plot(x_nf, d_nf, color="#D55E00")
        plt.fill_between(x_nf, d_nf, color="#D55E00", alpha=0.15)
...
def load_momenta(path_init, num_momenta_samples):
    """
    Load momenta from multiple sample folders.

    For each sample, the first row of the loaded
    momenta array is removed (empty row), and the remaining values are reshaped into an
    array of shape (720, 3).

    Parameters
    ----------
    path_init : str
        Path to the directory containing the momenta sample folders.
    num_momenta_samples : int
        Number of momenta samples to load.

    Returns
    -------
    np.ndarray
        Array containing all loaded momenta with shape
        (num_momenta_samples, 720, 3)
    """
    ...
    for i in range(num_momenta_samples):
        ...
        momenta = np.delete(momenta, 0, axis=0) ### delete first row (not any information in the first row)
...
def SVD(num_components, data_matrix):
    """
        Compute the singular value decomposition (SVD) of a data matrix and retain
        a reduced basis with a specified number of left singular vectors.

        The function performs SVD on `data_matrix`, extracts the first
        `num_components` left singular vectors, and reports the fraction of system
        energy captured by the selected singular values. It also prints the
        relative rank-r approximation error and the cumulative energy not captured
        by the truncated projection.

        Parameters
        ----------
        num_components : int
            Number of leading singular vectors to retain in the reduced basis.
        ...
        -------
        U : np.ndarray
            Matrix of left singular vectors from the full SVD.
        U_red : np.ndarray
            Reduced basis containing the first `num_components` columns of `U`.
        """
    ...
    system_energy = np.sum(s_selected**2) / np.sum(s**2)
...
def PCA(reference_group, generated_group, num_components, random_svd=False):
    """
    Project reference and generated data onto a PCA basis learned from the
    reference group.

    The function first mean-centers the reference and generated datasets. It
    then computes a reduced PCA basis from the reference group using either a
    full SVD or a randomized SVD. Finally, both the reference and generated
    groups are projected onto the reduced basis learned from the reference data.

    Parameters
    ----------
    reference_group : np.ndarray
        Reference data matrix used to compute the PCA basis
    generated_group : np.ndarray
        Generated data matrix to be projected onto the PCA basis learned from
        `reference_group`
    num_components : int
        Number of leading PCA components to retain.
    random_svd : bool, optional
        If True, compute the reduced basis using randomized SVD. If False, use
        the custom `SVD` function. Default is False.

    Returns
    -------
    X_rb_reference : np.ndarray
        PCA reduced-basis coefficients of the reference group
    X_rb_generated : np.ndarray
        PCA reduced-basis coefficients of the generated group

    Notes
    -----
    The PCA basis is computed only from the reference group. The generated group
    is then projected onto this same basis, allowing the two groups to be
    compared in a common reduced coordinate system.
    """
...
### First experiment
### For CAN-FLOW and the most performant cVAEs, we perform the PCA experiment, measure WDs and visualize them
...
### how many coefficients to visualize
num_coeffs_visual = 65
